...
export function KnowledgePage() {
  ...
  const handleViewDoc = async (doc: any) => {
    try {
      ...
      setViewDoc(doc);
      const res = await mockApi.getKnowledgeFileContent(doc.category, doc.name);
      ...
      <div className="animate-fade-up">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <button 
              onClick={() => navigate('/dashboard/knowledge/file/edit')}
              className="px-4 py-2 bg-gradient-to-r from-[#3b82f6] to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium text-xs rounded-xl transition-all shadow-md flex items-center gap-2 hover:-translate-y-0.5 active:translate-y-0"
            >
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/dashboard/knowledge/file/edit/${doc.category}/${doc.name}`);
                      }}
                      className="px-2.5 py-1.5 text-[10px] font-medium bg-white/5 border border-white/10 hover:bg-white/10 text-white/85 rounded-lg transition-all flex items-center gap-1.5"
                      title="Edit source document with AI in central editor"
                    >
